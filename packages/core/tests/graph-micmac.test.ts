...
describe("micmac", () => {
  ...
  it("yields an empty map for an empty graph", () => {
    const m = micmac({ nodes: [], edges: [] });
    expect(m.size).toBe(0);
  });
  it("classifies a single isolated node as autonomous", () => {
    const m = micmac({ nodes: [{ id: "a" }], edges: [] });
    expect(m.get("a")!.influence).toBe(0);
    expect(m.get("a")!.dependence).toBe(0);
    expect(m.get("a")!.quadrant).toBe("autonomous");
  });
});
